...
>
> **原则（v1.3）**：先 core 后 Jetson；**研发 Remote-SSH 在 Jetson 主验证**；Bootstrap = COCO stock + **Jetson 完整运行 UI**；场内数据与 `windows_studio` 后置。
>
...
Win: core+pytest      Jetson(Remote): TRT+UI 主验证   场内数据 + windows_studio
Jetson: M2 即刻       + 视频/USB + PLC 仿真→真机       + jetson_update 验收闸
  │                        │                              │
  ├ core ✅               ├ stock engine → M2            ├ 冻结测试集(微调前)
  ├ detect(Jetson 主)     ├ ui/ 监视/划区/报警           ├ studio 训练
  └ pipeline (Win 可选)   ├ M-Bootstrap                  └ E2E 热切换
                          └ 4路/USB/PLC真机/压测
```
...
2. **Jetson Remote**：stock ONNX → trtexec FP16 → **M2**（可与步骤 3 并行启动）  
3. `detect/backend` + **`trt_backend`（Jetson 主验证）**；Win 可选 `onnx_backend`  
4. **`app/main.py` + `ui/` 运行 UI** → **M-Bootstrap**（Jetson + 显示器/VNC）  
...
**两套 UI：** Jetson = **运行 UI**；Windows = **studio**。远程开发细则见 [validation_phases.md §1.6](validation_phases.md)。
...
| 4 | **Jetson** | `tools/export_onnx.sh` 在板上跑通 | ✅ |
...
- [ ] **Jetson**：stock FP16 engine + `trt_backend.infer()` 成功（**M2**）
- [ ] `detect/backend.py` 接口就位
- [ ] Win：`pytest tests/` 全绿
- [ ] Win `onnx_backend`：**不阻塞** M-Bootstrap
...
| **M3** | pipeline demo / CLI（Win 或 Jetson） |
...
| 顺序 | 任务 | 说明 |
|------|------|------|
| B1 | `app/main.py` 骨架 | 推理线程 + 视频源；**检测不在 UI 线程** |
| B2 | **`ui/` 最小可交付** | 预览、划区、运行/停、信号/报警、config 保存 |
| B3 | `camera/video_file.py` 接入 UI | D2；Bootstrap **首选输入** |
| B4 | PLC **仿真** | UI 显示拟写入 INT16；D11 |
| B5 | UI 标识 | 「STOCK · 集成测试 · 未过场内验收」 |
| B6 | （建议）`record/` 快照 | STOP 边沿落盘 |

> `trt_backend` + stock engine（M2）已在 Sprint 1.2 完成；本节从编排与 UI 开始。
...
1. [x] Sprint 1.1 core ✅  
2. [ ] **Jetson Day 0**：SSH 登录 → `git pull` → export + trtexec → **M2**  
3. [ ] Sprint 1.2：`backend` + `letterbox` + **`trt_backend`（Jetson）**  
4. [ ] Sprint 1.3：`pipeline` 接 TRT（Jetson 或 Win）  
5. [ ] Sprint B：`app/main.py` + **`ui/` → M-Bootstrap**（Jetson + 显示器/VNC）  
6. [ ] PLC 仿真（D11）→ 准备 `demo.mp4`（scp）  
7. [ ] `docs/benchmarks/jetson_bootstrap_*.md`  
8. [ ] USB / PLC 真机 / 4 路（M-Bootstrap 之后）  
9. [ ] 阶段三：场内冻结集 + studio  
10. [ ] （可选）Win `onnx_backend` 对照  
...
| §8.4 冻结集 | §5.0（微调模型前） |
| §10 路线 | §0.2、§3–5 |
| §12 待确认 | decisions.md D9–D12 |

---

*文档版本：v1.3 | 变更：D12 Jetson Remote 主验证；Sprint 1.2 Jetson-first；M2 并入 1.2 | 详见 validation_phases.md §1.6*
